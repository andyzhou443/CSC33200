...
int main (int argc, char* argv[]){
    ...
    int fd,fdDest,fdDestErr;// create file descriptors and an extra for error checking
    char buffer[8192]; //creates buffer to hold all read information 
    ...
    if (fd == -1){ //error checking for file access and outputs error if return error code
    ...
    if (fdDest == -1){ //error checking for file destination access and outputs error if return error code
    ...
    if (fd == -1){ //error checking for file can be open and outputs error if return error code
        printf("\n open() failed with error [%s]\n",strerror(errno)); 
        return 1; 
    }
    else{
        ...
        if(fd == -1){ //error checking for file can be readand outputs error if return error code
        ...
        if(fdDestErr== -1){  //error checking for file destination can be written and outputs error if return error code
        ...
        printf("\n");
        printf(" File from %s has been successfully copied into %s\n", filepath,fileDest); //if program is successful, output success messege

        close(fd); // closes fd to free space
        close(fdDest); // closes fdDest to free space
